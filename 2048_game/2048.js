...
document.addEventListener("keyup", (e) => {
  if (e.code == "ArrowLeft") {
    slideLeft();
    setTwo();
  } else if (e.code == "ArowRight") {
    slideRight();
    setTwo();
  } else if (e.code == "ArowUp") {
    slideUp();
    setTwo();
...
function slideUp() {
  for (let c = 0; c < columns; c++) {
    let row = [board[0][c], board[1][c], board[2][c], board[3][c]]; // convert column into row
    row = slide(row);
    // board[0][c]=row[0];// convert row into column
    // board[1][c]=row[1];
    // board[2][c]=row[2];
    // board[3][c]=row[3];
